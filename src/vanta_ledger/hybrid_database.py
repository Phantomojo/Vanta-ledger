...
class HybridDatabaseManager:
    ...
    def __init__(self):
        ...
        if not self.postgres_uri:
            raise ValueError(
                "POSTGRES_URI must be set in the environment for the hybrid database."
            )
        if not self.mongo_uri:
            raise ValueError(
                "MONGO_URI must be set in the environment for the hybrid database."
            )
    ...
    def _connect_databases(self):
        ...
        try:
            ...
            self.postgres_engine = create_engine(
                self.postgres_uri, pool_size=10, max_overflow=20, pool_pre_ping=True
            )
            logger.info("✅ Connected to PostgreSQL")

            # Connect to MongoDB
            self.mongo_client = MongoClient(
                self.mongo_uri, maxPoolSize=50, serverSelectionTimeoutMS=5000
            )
    ...
    def upload_document(
        self, file_data: Dict[str, Any], ai_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            mongo_doc = {
                ...
                "category": self._get_document_category(
                    ai_analysis.get("classification", "unknown")
                ),
                        ...
                        if postgres_id and postgres_id in postgres_data:
                            doc = {
                                **postgres_data[postgres_id],
                                "mongo_id": str(mongo_doc["_id"]),
                                "ai_analysis": mongo_doc.get("ai_analysis", {}),
                                "business_insights": mongo_doc.get(
                                    "business_insights", {}
                                ),
                                "tags": mongo_doc.get("tags", []),
                                "keywords": mongo_doc.get("keywords", []),
    ...
    def extract_financial_data_from_document(
        ...
        try:
            ...
            ledger_entry = {
                ...
                "category": self._get_transaction_category(
                    ai_analysis.get("classification", "unknown")
                ),
    ...
    def _calculate_tax_rate(self, amounts: List[Dict[str, Any]]) -> Optional[float]:
        ...
        tax_amount = self._extract_tax_amount(amounts)
        subtotal = sum(
            amount["value"] for amount in amounts if amount.get("type") == "subtotal"
        )
