...
async def predict_financial_metrics(
    forecast_periods: int = Query(12, ge=1, le=24, description="Number of periods to forecast"),
    current_user: User = Depends(get_current_user)
):
...
async def detect_anomalies(
    data_type: str = Query("all", description="Type of data to analyze: all, financial, documents, payments"),
    current_user: User = Depends(get_current_user)
):
...
async def get_analytics_dashboard(
    ...
    try:
        ...
        trends = await enhanced_ai_analytics_service.analyze_financial_trends(current_user.id, 90)
        predictions = await enhanced_ai_analytics_service.predict_financial_metrics(current_user.id, 6)
        anomalies = await enhanced_ai_analytics_service.detect_anomalies(current_user.id, "all")
        insights = await enhanced_ai_analytics_service.generate_financial_insights(current_user.id)
