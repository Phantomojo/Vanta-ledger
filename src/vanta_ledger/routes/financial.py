...
from ..models.financial_models import (
    ChartOfAccounts, JournalEntry, Invoice, Customer, Vendor,
    AccountType, Currency, InvoiceStatus, PaymentStatus
)
...
async def get_account_types():
    ...
    return {
        "success": True,
        "account_types": [at.value for at in AccountType]
    }
...
async def list_journal_entries(
    ...
    try:
        entries, total_count = financial_service.get_journal_entries(page, limit, is_posted)
...
async def get_journal_entry(
    ...
    try:
        ...
        # Get journal entry
        entry_data = financial_service.journal_entries.find_one({"_id": str(entry_uuid)})
        if not entry_data:
            raise HTTPException(
        ...
        # Get entry lines
        lines_data = financial_service.journal_entry_lines.find({"journal_entry_id": str(entry_uuid)})
...
async def list_invoices(
    ...
    try:
        ...
        invoices, total_count = financial_service.get_invoices(customer_uuid, status_enum, page, limit)
...
async def get_invoice(
    ...
    try:
        ...
        # Get invoice lines
        lines_data = financial_service.invoice_lines.find({"invoice_id": str(invoice_uuid)})
...
async def get_customer(
    ...
    try:
        ...
        customer_data = financial_service.customers.find_one({"_id": str(customer_uuid)})
...
async def validate_account_code(
    ...
    try:
        ...
        # Check uniqueness
        existing_account = financial_service.chart_of_accounts.find_one({"account_code": account_code.upper()})
        if existing_account:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def validate_customer_code(
    ...
    try:
        existing_customer = financial_service.customers.find_one({"customer_code": customer_code})
        ...
        return {
            "success": True,
            "valid": True,
            "message": "Customer code is available"
        }
    except HTTPException:
        raise
    except Exception as e:
