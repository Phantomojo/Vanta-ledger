...
from ..models.document_models import (
    DocumentSearchCriteria, DocumentTag, DocumentCategory, 
    DocumentType, DocumentStatus, DocumentPriority
)
...
async def create_enhanced_document(
    ...
    try:
        document = enhanced_document_service.create_document(document_data, current_user.id)
        return {
            "success": True,
...
async def list_documents(
    ...
    try:
        ...
        if category_id:
            criteria.category_id = input_validator.validate_uuid(category_id, "category_id")
        
        if created_by:
            criteria.created_by = input_validator.validate_uuid(created_by, "created_by")
        
        # Execute search
        documents, total_count = enhanced_document_service.search_documents(criteria, current_user.id)
...
async def advanced_search(
    ...
    try:
        documents, total_count = enhanced_document_service.search_documents(search_criteria, current_user.id)
        ...
        return {
            ...
            "pagination": {
                ...
                "total": total_count,
                "total_pages": (total_count + search_criteria.limit - 1) // search_criteria.limit
            }
        }
    except Exception as e:
        raise HTTPException(
...
async def create_category(
    ...
    try:
        category = enhanced_document_service.create_category(category_data, current_user.id)
        return {
            "success": True,
...
async def list_categories(
    include_system: bool = Query(True, description="Include system-generated categories"),
    current_user: User = Depends(get_current_user)
):
    """List all document categories"""
    try:
        categories = enhanced_document_service.get_categories(include_system=include_system)
        return {
            "success": True,
...
async def get_document_statuses():
    ...
    return {
        "success": True,
        "statuses": [ds.value for ds in DocumentStatus]
    }
...
async def get_document_priorities():
    ...
    return {
        "success": True,
        "priorities": [dp.value for dp in DocumentPriority]
    }
...
async def get_search_suggestions(
    ...
    try:
        ...
        suggestions = {
            "document_types": [dt.value for dt in DocumentType if query.lower() in dt.value.lower()],
            "tags": [],
            "categories": []
        }
        ...
        suggestions["tags"] = [tag.name for tag in tags if query.lower() in tag.name.lower()]
        
        # Get matching categories
        categories = enhanced_document_service.get_categories()
        suggestions["categories"] = [cat.name for cat in categories if query.lower() in cat.name.lower()]
