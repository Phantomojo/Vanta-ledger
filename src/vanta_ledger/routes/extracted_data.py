...
async def get_extracted_data(
    ...
    try:
        ...
        cursor = collection.find(query_filter).skip(skip).limit(limit).sort("created_date", -1)
...
async def get_extracted_data_analytics(current_user: dict = Depends(AuthService.verify_token)):
...
@router.get("/stats")
async def get_extracted_data_stats(current_user: dict = Depends(AuthService.verify_token)):
    ...
    try:
        ...
        return {
            "categories": [{"category": item["_id"] or "Unknown", "count": item["count"]} for item in categories],
            "transaction_types": [{"type": item["_id"] or "Unknown", "count": item["count"]} for item in transaction_types],
            "confidence_distribution": {
                "high": collection.count_documents({"confidence": {"$gte": 0.8}}),
                "medium": collection.count_documents({"confidence": {"$gte": 0.5, "$lt": 0.8}}),
