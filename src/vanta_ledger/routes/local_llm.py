...
async def process_document_with_llm(
    ...
    try:
        ...
        temp_fd, temp_path = tempfile.mkstemp(
            suffix=f"_{file.filename}", prefix=f"user_{current_user.id}_"
        )
...
async def analyze_text_with_llm(
    ...
    analysis_type: str = Query(
        "general", description="Type of analysis: general, financial, entities"
    ),
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        ...
        # Get company context
        company_context = (
            await local_llm_service.company_context_manager.get_company_context(
                company_uuid
            )
        )
        ...
        if analysis_type == "financial":
            result = await local_llm_service._extract_financial_data_with_context(
                text, company_context
            )
        elif analysis_type == "entities":
            result = await local_llm_service._extract_entities_with_context(
                text, company_context
            )
        else:
            result = {
                "classification": await local_llm_service._classify_document_with_context(
                    text, company_context
                ),
                "summary": await local_llm_service._generate_summary_with_context(
                    text, company_context
                ),
            }
...
async def get_company_context(
    ...
    try:
        ...
        context = await local_llm_service.company_context_manager.get_company_context(
            company_uuid
        )

        return {"success": True, "company_context": context}
...
async def get_company_statistics(
    ...
    try:
        ...
        stats = local_llm_service.company_context_manager.get_company_statistics(
            company_uuid
        )
...
async def get_hardware_status(current_user: User = Depends(get_current_user)):
    """Get current hardware status and utilization"""
    try:
        status = await local_llm_service.get_hardware_status()

        return {"success": True, "hardware_status": status}
...
async def get_hardware_summary(current_user: User = Depends(get_current_user)):
...
async def get_model_status(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        recommended_models = local_llm_service.hardware_config.get(
            "recommended_models", {}
        )
        ...
        return {
            ...
            "hardware_profile": local_llm_service.hardware_config[
                "performance_profile"
            ],
        }
...
async def clear_llm_cache(current_user: User = Depends(get_current_user)):
    """Clear LLM cache"""
    try:
...
async def get_llm_diagnostics(current_user: User = Depends(get_current_user)):
    ...
    try:
        diagnostics = {
            ...
            "models": {
                ...
                "recommended": local_llm_service.hardware_config.get(
                    "recommended_models", {}
                ),
