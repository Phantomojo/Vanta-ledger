...
class CompanyContextManager:
    ...
    def build_company_prompt_context(self, company_context: Dict) -> str:
        ...
        if company_context.get("customers"):
            customer_list = ", ".join(
                [f"{c['code']}: {c['name']}" for c in company_context["customers"][:5]]
            )
        ...
        if company_context.get("vendors"):
            vendor_list = ", ".join(
                [f"{v['code']}: {v['name']}" for v in company_context["vendors"][:5]]
            )
        ...
        if company_context.get("financial_accounts"):
            account_list = ", ".join(
                [
                    f"{a['code']}: {a['name']}"
                    for a in company_context["financial_accounts"][:10]
                ]
            )
    ...
    def get_company_specific_instructions(
        self, company_context: Dict, task_type: str
    ) -> str:
    ...
    def validate_entity_against_context(
        self, entity: str, entity_type: str, company_context: Dict
    ) -> Dict[str, Any]:
        ...
        try:
            if entity_type == "customer":
                ...
                for customer in customers:
                    if (
                        entity.lower() in customer["name"].lower()
                        or entity.lower() in customer["code"].lower()
                    ):
                ...
                for vendor in vendors:
                    if (
                        entity.lower() in vendor["name"].lower()
                        or entity.lower() in vendor["code"].lower()
                    ):
                        validation_result.update(
                            {
                                "is_valid": True,
                ...
                if currency == "KES":
                    ...
                    if (
                        "KSh" in entity
                        or "KES" in entity
                        or any(c.isdigit() for c in entity)
                    ):
                        validation_result.update({"is_valid": True, "confidence": 0.8})
    ...
    def get_company_processing_rules(
        self, company_context: Dict, document_type: str
    ) -> Dict[str, Any]:
        ...
        if document_rules:
            ...
            for key, value in document_rules.items():
                if (
                    key in final_rules
                    and isinstance(final_rules[key], dict)
