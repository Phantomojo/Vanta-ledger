...
from ..models.document_models import (
    EnhancedDocument, DocumentTag, DocumentCategory, DocumentVersion,
    DocumentSearchCriteria, DocumentStatus, DocumentType
)
...
class EnhancedDocumentService:
    ...
    def create_document(self, document_data: Dict[str, Any], user_id: UUID) -> EnhancedDocument:
    ...
    def get_document(self, document_id: UUID, user_id: UUID) -> Optional[EnhancedDocument]:
    ...
    def search_documents(self, criteria: DocumentSearchCriteria, user_id: UUID) -> Tuple[List[EnhancedDocument], int]:
        ...
        try:
            ...
            if criteria.full_text:
                ...
                text_results = list(self.search_index.find(
                    {"$text": {"$search": criteria.full_text}},
                    {"_id": 1}
                ))
            ...
            if criteria.description:
                query["metadata.description"] = {"$regex": criteria.description, "$options": "i"}
            
            # Classification filters
            if criteria.document_types:
                query["metadata.document_type"] = {"$in": [dt.value for dt in criteria.document_types]}
            if criteria.tags:
                query["metadata.tags"] = {"$in": [str(tag_id) for tag_id in criteria.tags]}
            ...
            if criteria.priority:
                query["metadata.priority"] = {"$in": [p.value for p in criteria.priority]}
            ...
            cursor = self.documents.find(query).sort(sort_field, sort_order).skip(skip).limit(criteria.limit)
    ...
    def create_tag(self, tag_data: Dict[str, Any], user_id: UUID) -> DocumentTag:
        ...
        try:
            # Validate input
            tag_data = input_validator.validate_json_payload(tag_data, required_fields=["name"])
    ...
    def create_category(self, category_data: Dict[str, Any], user_id: UUID) -> DocumentCategory:
        """Create a new document category"""
        try:
            # Validate input
            category_data = input_validator.validate_json_payload(category_data, required_fields=["name"])
            
            # Check if category already exists
            existing_category = self.categories.find_one({"name": category_data["name"]})
    ...
    def get_document_statistics(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            for doc_type in DocumentType:
                count = self.documents.count_documents({"metadata.document_type": doc_type.value})
            ...
            stats["total_storage_bytes"] = size_result[0]["total_size"] if size_result else 0
    ...
    async def create_document_with_llm(self, document_data: Dict[str, Any], user_id: UUID, company_id: UUID) -> EnhancedDocument:
        ...
        try:
            ...
            if document.extracted_text:
                try:
                    ...
                    llm_results = await local_llm_service.process_document_for_company(document, company_id)
                    
                    # Update document with LLM insights
                    if llm_results:
                        # Update classification
                        if 'classification' in llm_results:
                            document.metadata['llm_classification'] = llm_results['classification']
                            document.metadata['document_type'] = llm_results['classification'].get('type', 'unknown')
                        ...
                        if 'financial_data' in llm_results:
                            document.metadata['llm_financial_data'] = llm_results['financial_data']
                        
                        # Update document understanding
                        if 'document_understanding' in llm_results:
                            document.metadata['llm_document_understanding'] = llm_results['document_understanding']
                        
                        # Add LLM processing timestamp
                        document.metadata['llm_processed_at'] = datetime.utcnow().isoformat()
                        ...
                        logger.info(f"Document {document.id} processed with LLM for company {company_id}")
