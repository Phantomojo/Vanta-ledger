...
from ..models.financial_models import (
    ChartOfAccounts, JournalEntry, JournalEntryLine, AccountBalance,
    Invoice, InvoiceLine, Bill, Payment, PaymentAllocation,
    Customer, Vendor, AccountType, Currency, InvoiceStatus, PaymentStatus
)
...
class FinancialService:
    ...
    def _initialize_default_accounts(self):
        ...
        try:
            default_accounts = [
                ...
                {"code": "5200", "name": "Payroll Expenses", "type": AccountType.EXPENSE}
            ]
            
            for account_data in default_accounts:
                if not self.chart_of_accounts.find_one({"account_code": account_data["code"]}):
    ...
    def create_account(self, account_data: Dict[str, Any], user_id: UUID) -> ChartOfAccounts:
        """Create a new chart of accounts entry"""
        try:
            account_data = input_validator.validate_json_payload(account_data, required_fields=["account_code", "account_name", "account_type"])
            
            # Check if account code already exists
            existing_account = self.chart_of_accounts.find_one({"account_code": account_data["account_code"]})
            if existing_account:
    ...
    def get_accounts(self, account_type: Optional[AccountType] = None, include_inactive: bool = False) -> List[ChartOfAccounts]:
    ...
    def create_journal_entry(self, entry_data: Dict[str, Any], user_id: UUID) -> JournalEntry:
        """Create a new journal entry"""
        try:
            entry_data = input_validator.validate_json_payload(entry_data, required_fields=["entry_number", "entry_date", "description", "lines"])
    ...
    def get_journal_entries(self, page: int = 1, limit: int = 20, is_posted: Optional[bool] = None) -> Tuple[List[JournalEntry], int]:
        ...
        try:
            ...
            cursor = self.journal_entries.find(query).sort("entry_date", -1).skip(skip).limit(limit)
    ...
    def create_invoice(self, invoice_data: Dict[str, Any], user_id: UUID) -> Invoice:
        ...
        try:
            invoice_data = input_validator.validate_json_payload(invoice_data, required_fields=["invoice_number", "customer_id", "invoice_date", "due_date", "lines"])
    ...
    def get_invoices(self, customer_id: Optional[UUID] = None, status: Optional[InvoiceStatus] = None, 
                    page: int = 1, limit: int = 20) -> Tuple[List[Invoice], int]:
        ...
        try:
            ...
            cursor = self.invoices.find(query).sort("invoice_date", -1).skip(skip).limit(limit)
    ...
    def create_customer(self, customer_data: Dict[str, Any], user_id: UUID) -> Customer:
        ...
        try:
            customer_data = input_validator.validate_json_payload(customer_data, required_fields=["customer_code", "customer_name"])
            
            # Check if customer code already exists
            existing_customer = self.customers.find_one({"customer_code": customer_data["customer_code"]})
    ...
    def get_financial_statistics(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            # Account statistics
            stats["total_accounts"] = self.chart_of_accounts.count_documents({"is_active": True})
            ...
            stats["posted_entries"] = self.journal_entries.count_documents({"is_posted": True})
            stats["unposted_entries"] = self.journal_entries.count_documents({"is_posted": False})
            
            # Customer statistics
            stats["total_customers"] = self.customers.count_documents({"is_active": True})
