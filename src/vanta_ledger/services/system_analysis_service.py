...
class SystemAnalysisService:
    ...
    def __init__(self):
        ...
        if self.enabled:
            ...
            logger.warning("System Analysis Service disabled - GitHub Models not available")
    
    async def analyze_system_health(self, include_logs: bool = True, log_lines: int = 1000) -> Dict[str, Any]:
        ...
        try:
            ...
            messages = github_models_service._render_prompt_template('system_health_analyzer', variables)
            ...
            response = await github_models_service._make_request(messages, model=model, **model_params)
    ...
    async def analyze_code_quality(self, file_path: str, context: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            messages = github_models_service._render_prompt_template('code_reviewer', variables)
    ...
    async def analyze_project_codebase(self, project_dir: str = ".", include_patterns: List[str] = None, 
        ...
        exclude_patterns = exclude_patterns or ['__pycache__', 'node_modules', '.git', '*.pyc', 'venv', '.venv']
        ...
        try:
            ...
            for pattern in include_patterns:
                for file_path in project_path.rglob(pattern):
                    ...
                    if file_path.is_file() and file_path.stat().st_size < 100000:  # Limit to files < 100KB
            ...
            for file_path in code_files:
                try:
                    ...
                    if 'code_quality_issues' in file_analysis:
                        overall_issues['code_quality'] += len(file_analysis['code_quality_issues'])
                    
                    if 'performance_issues' in file_analysis:
                        overall_issues['performance'] += len(file_analysis['performance_issues'])
            ...
            return {
                ...
                'recommendations': self._generate_project_recommendations(overall_issues),
                'analysis_metadata': {
                    'analyzed_at': datetime.utcnow().isoformat(),
    ...
    def _generate_project_recommendations(self, issues: Dict[str, int]) -> List[Dict[str, str]]:
