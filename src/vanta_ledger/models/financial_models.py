...
class ChartOfAccounts(BaseModel):
    ...
    def validate_account_code(cls, v):
        ...
        if not v.replace('-', '').replace('.', '').isalnum():
            raise ValueError('Account code must be alphanumeric with optional hyphens and dots')
        return v.upper()
...
class Payment(BaseModel):
    ...
    payment_type: str = Field(..., min_length=1, max_length=50)  # cash, bank, card, etc.
...
class FinancialReport(BaseModel):
    ...
    report_type: str = Field(..., min_length=1, max_length=50)  # P&L, Balance Sheet, Cash Flow
