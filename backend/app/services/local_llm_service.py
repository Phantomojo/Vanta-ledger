...
try:
    ...
    try:
        ...
        logging.warning("LayoutLMv3 not available - document layout understanding disabled")
except ImportError:
...
class LocalLLMService:
    ...
    def __init__(self):
        ...
        logger.info(f"Local LLM Service initialized with hardware: {self.hardware_detector.get_hardware_summary()}")
    ...
    def _load_model_configs(self) -> Dict[str, Dict]:
        ...
        for model_name, config in base_configs.items():
            if model_name in recommended_models:
                ...
                config["context_length"] = recommended_config.get("context_length", config["context_length"])
                config["batch_size"] = recommended_config.get("batch_size", 1)
                config["quantization"] = recommended_config.get("quantization", "Q4_K_M")
    ...
    async def initialize_models(self):
        ...
        try:
            ...
            primary_model = self.hardware_config["recommended_models"]["primary"]["name"]
            await self._load_model(primary_model)
            
            # Load secondary model if different
            secondary_model = self.hardware_config["recommended_models"]["secondary"]["name"]
            ...
            if "document_understanding" in self.hardware_config["recommended_models"]:
                doc_model = self.hardware_config["recommended_models"]["document_understanding"]["name"]
    ...
    async def _load_layoutlm_model(self, config: Dict):
        ...
        try:
            if not LAYOUTLM_AVAILABLE:
                logger.warning("LayoutLMv3 not available, skipping document layout model")
                return
    ...
    async def process_document_for_company(self, document: EnhancedDocument, company_id: UUID) -> Dict[str, Any]:
        """Process document with company-specific context"""
        try:
            start_time = time.time()
            
            # Get company context
            company_context = await self.company_context_manager.get_company_context(company_id)
            ...
            results = await self._process_with_company_context(document, company_context)
            ...
            self._record_performance_metrics("company_document_processing", processing_time)
    ...
    async def _process_with_company_context(self, document: EnhancedDocument, company_context: Dict) -> Dict[str, Any]:
        """Process document with company-specific context"""
        results = {}
        
        # Build company-specific prompt context
        context_prompt = self.company_context_manager.build_company_prompt_context(company_context)
    ...
    async def _classify_document_with_context(self, text: str, company_context: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            instructions = self.company_context_manager.get_company_specific_instructions(
    ...
    def _parse_classification_result(self, result: str, company_context: Dict) -> Dict[str, Any]:
    ...
    async def _generate_summary_with_context(self, text: str, company_context: Dict) -> str:
        """Generate summary with company context"""
        try:
            primary_model = self.hardware_config["recommended_models"]["primary"]["name"]
            if primary_model not in self.models:
    ...
    async def _extract_entities_with_context(self, text: str, company_context: Dict) -> Dict[str, List[str]]:
        """Extract entities with company context"""
        try:
            primary_model = self.hardware_config["recommended_models"]["primary"]["name"]
            if primary_model not in self.models:
    ...
    def _filter_entities_by_context(self, entities: Dict, company_context: Dict) -> Dict:
        """Filter entities based on company context"""
        ...
        if company_context["company_name"] in entities.get("companies", []):
            ...
            ] + [c for c in entities.get("companies", []) if c != company_context["company_name"]]
        
        # Filter account codes based on company's chart of accounts
        company_accounts = [acc["code"] for acc in company_context.get("financial_accounts", [])]
        if company_accounts:
            filtered_entities["account_codes"] = [
                code for code in entities.get("account_codes", [])
                if code in company_accounts
            ]
        
        return filtered_entities
    
    def _simple_entity_extraction_with_context(self, text: str, company_context: Dict) -> Dict[str, List[str]]:
    ...
    async def _extract_financial_data_with_context(self, text: str, company_context: Dict) -> Dict[str, Any]:
        """Extract financial data with company context"""
        try:
            ...
            try:
                ...
                if json_match:
                    ...
                    return self._validate_financial_data(financial_data, company_context)
            except:
                pass
    ...
    def _validate_financial_data(self, financial_data: Dict, company_context: Dict) -> Dict:
        ...
        if financial_data.get("customer_name"):
            ...
            if financial_data["customer_name"] not in customers + vendors:
                validated_data["customer_name"] = f"Unknown: {financial_data['customer_name']}"
        
        # Validate account codes
        if financial_data.get("account_codes"):
            company_accounts = [acc["code"] for acc in company_context.get("financial_accounts", [])]
            validated_data["account_codes"] = [
                code for code in financial_data["account_codes"]
                if code in company_accounts
            ]
