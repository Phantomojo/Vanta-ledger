...
try:
    ...
    venv_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__)))), 'venv', 'lib', 'python3.12', 'site-packages')
...
class HardwareDetector:
    ...
    def detect_hardware(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Hardware detection completed: {self.detected_hardware['performance_profile']}")
    ...
    def _detect_cpu(self):
        ...
        try:
            ...
            logger.info(f"CPU detected: {cpu_count} cores, {cpu_freq.current if cpu_freq else 'unknown'} MHz")
    ...
    def get_hardware_summary(self) -> str:
        ...
        if self.gpu_info:
            summary_parts.append(f"GPU: {self.gpu_info['name']} ({self.gpu_info['memory_total']}MB)")
        ...
        summary_parts.append(f"Profile: {self.detected_hardware.get('performance_profile', 'unknown')}")
