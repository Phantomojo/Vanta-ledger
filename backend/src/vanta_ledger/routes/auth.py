...
async def get_me(current_user: dict = Depends(verify_token)):
    """
    Return the current authenticated user's basic profile information.
    """
    try:
        username = current_user.get("sub") or current_user.get("username")
        user_id = current_user.get("user_id") or 0
        role = current_user.get("role", "user")
        email = current_user.get("email") or (f"{username}@example.com" if username else "user@example.com")
        return {
            "id": user_id,
            "username": username,
            "email": email,
