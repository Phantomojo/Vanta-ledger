...
from ..models.document_models import DocumentStatus, DocumentType, EnhancedDocument
from ..models.financial_models import AccountType, ChartOfAccounts, Customer, Invoice
...
class VantaLedgerSystemIntegrator:
    ...
    async def get_system_overview(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get AI insights
            ai_insights = await self.ai_analytics_service.generate_financial_insights(
                user_id
            )
    ...
    async def process_document_with_financial_integration(
        ...
        try:
            ...
            if financial_data.get("is_invoice"):
                invoice = await self._create_invoice_from_document(
                    document, financial_data, user_id
                )
                financial_records["invoice"] = invoice

            if financial_data.get("is_receipt"):
                journal_entry = await self._create_journal_entry_from_document(
                    document, financial_data, user_id
                )
            ...
            logger.error(
                f"Error processing document with financial integration: {str(e)}"
            )
            raise

    async def _extract_financial_data_from_document(
        self, document: EnhancedDocument
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if document.extracted_text:
                ...
                if any(
                    keyword in text
                    for keyword in ["receipt", "payment", "paid", "amount paid"]
                ):
    ...
    async def _create_invoice_from_document(
        self, document: EnhancedDocument, financial_data: Dict[str, Any], user_id: UUID
        ...
        try:
            ...
            invoice_data = {
                ...
                "lines": [
                    {
                        "item_description": document.metadata.title
    ...
    async def _create_journal_entry_from_document(
        self, document: EnhancedDocument, financial_data: Dict[str, Any], user_id: UUID
    ) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            cash_account = next(
                (acc for acc in cash_accounts if "cash" in acc.account_name.lower()),
                None,
            )
            ...
            entry_data = {
                ...
                "lines": [
                    ...
                    },
                    {
                        "account_id": str(
                            cash_account.id
                        ),  # Placeholder for revenue account
                        "description": "Revenue",
                        "debit_amount": "0.00",
            ...
            journal_entry = self.financial_service.create_journal_entry(
                entry_data, user_id
            )
    ...
    def _update_document_with_financial_metadata(
        self, document: EnhancedDocument, financial_data: Dict[str, Any]
    ):
        ...
        try:
            ...
            if financial_data.get("is_invoice"):
                ...
                invoice_tag = next(
                    (tag for tag in invoice_tags if "invoice" in tag.name.lower()), None
                )
            ...
            if financial_data.get("is_receipt"):
                ...
                receipt_tag = next(
                    (tag for tag in receipt_tags if "receipt" in tag.name.lower()), None
                )
            ...
            if financial_data.get("amount"):
                document.metadata.custom_fields["extracted_amount"] = financial_data[
                    "amount"
                ]
                document.metadata.custom_fields["extracted_currency"] = financial_data[
                    "currency"
                ]
    ...
    async def get_financial_dashboard(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            trends = await self.ai_analytics_service.analyze_financial_trends(
                user_id, 90
            )

            # Get predictions
            predictions = await self.ai_analytics_service.predict_financial_metrics(
                user_id, 6
            )

            # Get anomalies
            anomalies = await self.ai_analytics_service.detect_anomalies(
                user_id, "financial"
            )

            # Get insights
            insights = await self.ai_analytics_service.generate_financial_insights(
                user_id
            )
    ...
    async def get_document_analytics(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            doc_anomalies = await self.ai_analytics_service.detect_anomalies(
                user_id, "documents"
            )
    ...
    async def _analyze_document_processing(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_documents = list(
                self.document_service.documents.find(
                    {
            ...
            for doc in recent_documents:
                if doc.get("processing_started_at") and doc.get(
    ...
    def _calculate_processing_success_rate(
    ...
    async def run_system_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Cache warmup
            warmup_functions = [
                lambda: self.document_service.get_document_statistics(
                    UUID(1)
                ),  # Placeholder for a valid UUID
                lambda: self.financial_service.get_financial_statistics(
                    UUID(1)
                ),  # Placeholder for a valid UUID
            ]
    ...
    async def export_system_report(self, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            report = {
                ...
                "financial_dashboard": financial_dashboard.get(
                    "financial_dashboard", {}
                ),
