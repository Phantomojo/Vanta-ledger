...
class InputValidator:
    ...
    def validate_email(self, value: Any, field_name: str = "email") -> str:
        ...
        value = self.validate_string(
            value, min_length=5, max_length=254, field_name=field_name
        )
    ...
    def validate_filename(self, value: Any, field_name: str = "filename") -> str:
        ...
        value = self.validate_string(
            value, min_length=1, max_length=255, field_name=field_name
        )
    ...
    def validate_pagination_params(
        self, page: Any, limit: Any, max_limit: int = 1000
    ) -> tuple[int, int]:
        """Validate pagination parameters"""
        page = self.validate_integer(page, min_value=1, field_name="page")
        limit = self.validate_integer(
            limit, min_value=1, max_value=max_limit, field_name="limit"
        )
    ...
    def sanitize_sql_identifier(self, value: str) -> str:
        ...
        if not re.match(r"^[a-zA-Z_][a-zA-Z0-9_]*$", value):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid SQL identifier"
            )
        return value
