...
class Settings:
    ...
    # Development convenience: allow starting without DBs (for test endpoints)
    ALLOW_MISSING_DATABASES: bool = os.getenv("ALLOW_MISSING_DATABASES", "True").lower() == "true"
    ...
    LOGIN_RATE_LIMIT_PER_MINUTE: int = int(
        os.getenv("LOGIN_RATE_LIMIT_PER_MINUTE", "5")
    )
    TRUST_PROXY: bool = os.getenv("TRUST_PROXY", "False").lower() == "true"

    # File Storage
    UPLOAD_DIR: str = os.getenv("UPLOAD_DIR", "data/uploads")
    PROCESSED_DOCUMENTS_DIR: str = os.getenv(
    ...
    # Feature Flags
    ENABLE_GITHUB_MODELS: bool = os.getenv("ENABLE_GITHUB_MODELS", "True").lower() == "true"

    def validate_required_config(self):
        """
        Validate required runtime configuration at application startup.
        In development or when ALLOW_MISSING_DATABASES is true, allow app to start without DB URIs.
        Always ensure SECRET_KEY is set; if missing, generate an ephemeral one and warn.
        """
        is_debug = self.DEBUG

        # Ensure SECRET_KEY is present; generate ephemeral only in DEBUG, fail in production
        if not self.SECRET_KEY:
            if is_debug:
                self.SECRET_KEY = secrets.token_urlsafe(64)
                logging.warning(
                    "SECRET_KEY missing; generated an ephemeral key for DEBUG run. Do not use this in production."
                )
            else:
                raise ValueError("SECRET_KEY must be set in production.")

        # Handle database URIs based on environment
        missing_mongo = not bool(self.MONGO_URI)
        missing_pg = not bool(self.POSTGRES_URI)
        if missing_mongo or missing_pg:
            if is_debug or self.ALLOW_MISSING_DATABASES:
                logging.warning(
                    "Database URIs are missing (MONGO_URI=%s, POSTGRES_URI=%s). Running without DB; only test/static endpoints will work.",
                    not missing_mongo,
                    not missing_pg,
                )
            else:
