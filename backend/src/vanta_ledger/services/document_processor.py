...
try:
    ...
    OCR_AVAILABLE = False
    logging.warning(
        "OCR libraries not available. Install: pip install pytesseract pdf2image python-docx PyMuPDF Pillow"
    )
...
try:
    ...
    NLP_AVAILABLE = False
    logging.warning(
        "spaCy library not available. Install: pip install spacy && python -m spacy download en_core_web_sm"
    )
else:
    try:
...
class DocumentProcessor:
    ...
    def __init__(
        ...
        # Use settings if available, otherwise use current working directory
        ...
        # Convert to absolute paths, handling relative paths properly
        if upload_dir.startswith('./'):
            upload_dir = upload_dir[2:]  # Remove ./ prefix
        
        # Special case: if upload_dir is just "uploads", use "data/uploads" instead
        ...
        if Path(upload_dir).is_absolute():
            self.upload_dir = Path(upload_dir)
        else:
            # For relative paths, always resolve from project root
            self.upload_dir = (project_root / upload_dir).resolve()
            
        if Path(processed_dir).is_absolute():
            self.processed_dir = Path(processed_dir)
        else:
            # For relative paths, always resolve from project root
            self.processed_dir = (project_root / processed_dir).resolve()
