...
try:
    from ..database import postgres_ping, mongo_ping, redis_ping
except Exception:
    # Allow service to run even if database drivers are missing
    def postgres_ping(*args, **kwargs):
        return {"status": "skipped", "reason": "unavailable"}
    def mongo_ping(*args, **kwargs):
        return {"status": "skipped", "reason": "unavailable"}
    def redis_ping(*args, **kwargs):
        return {"status": "skipped", "reason": "unavailable"}
...
class SystemAnalysisService:
    ...
    def _collect_db_health(self) -> Dict[str, Any]:
        """Collect health for configured databases with latency and version"""
        try:
            return {
            ...
            logger.warning(f"DB health collection failed: {e}")
            return {"error": str(e), "timestamp": datetime.utcnow().isoformat()}

    def get_snapshot(self) -> Dict[str, Any]:
        """Quick snapshot without AI: system metrics + db health"""
        sys_metrics = self._collect_system_metrics()
        db_metrics = self._collect_db_health()
        return {
            "status": "ok",
            "system": sys_metrics,
    ...
    async def analyze_system_health(
        ...
        try:
            ...
            db_health = self._collect_db_health()
            variables = {
                "timestamp": datetime.utcnow().isoformat(),
                "environment": os.environ.get("ENVIRONMENT", "unknown"),
                "metrics_data": json.dumps(metrics_data, indent=2),
                "db_health": json.dumps(db_health, indent=2),
            ...
            try:
                ...
                # Also surface db health at top level for convenience
                analysis.setdefault("db_health", db_health)
