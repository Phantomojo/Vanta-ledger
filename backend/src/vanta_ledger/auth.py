...
if _HAS_REDIS:
    ...
    # Fallback no-op client to allow running without Redis during tests
    class _NullRedis:
        def setex(self, *args, **kwargs):
            return True
        def exists(self, *args, **kwargs):
    ...
    def authenticate_user(username: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                db_user = (
                    session.query(UserDB).filter(UserDB.username == username).first()
                )
...
def get_user_by_username(username: str) -> Optional[User]:
    ...
    try:
        ...
        logger.error(
            f"Error getting user by username {username}: User retrieval failed"
        )
        return None
