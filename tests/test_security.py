...
class TestAuthentication:
    ...
    def test_login_with_valid_credentials(self):
        ...
        pwd = get_admin_password()
        response = client.post(
            "/auth/login", data={"username": "admin", "password": pwd}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_login_with_sql_injection(self):
        ...
        for payload in sql_injection_payloads:
            response = client.post(
                "/auth/login", data={"username": payload, "password": "password"}
            )
    ...
    def test_jwt_token_validation(self):
        ...
        response = client.get(
            "/health", headers={"Authorization": f"Bearer {expired_token}"}
        )
        assert response.status_code == 401
...
class TestInputValidation:
    ...
    def test_string_validation(self):
        ...
        assert (
            input_validator.validate_string("  hello  ") == "hello"
        )  # Trim whitespace
    ...
    def test_filename_validation(self):
        ...
        valid_filenames = ["document.pdf", "file_name.docx", "test123.txt"]
...
class TestFileUploadSecurity:
    ...
    def test_secure_filename_generation(self):
        ...
        secure_filename = secure_file_handler.generate_secure_filename(
        ...
        # Should be unique
        secure_filename2 = secure_file_handler.generate_secure_filename(
            original_filename, user_id
        )
