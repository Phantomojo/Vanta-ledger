...
class CoreFunctionalityTestSuite:
    ...
    def test_system_environment(self) -> bool:
        ...
        try:
            ...
            self.system_info = {
                ...
                "torch_version": torch_version if 'torch_version' in locals() else "Not installed",
                "transformers_version": transformers_version if 'transformers_version' in locals() else "Not installed",
                "cpu_count": cpu_count,
            ...
            self.log_test_result("System Environment", success, details, self.system_info)
            return success
    ...
    def test_model_files(self) -> bool:
        ...
        try:
            ...
            if exists:
                ...
                success = exists and readable and size_mb > 100  # Should be at least 100MB
    ...
    def test_llm_functionality(self) -> bool:
        ...
        try:
            ...
            response_text = response['choices'][0]['text'] if 'choices' in response else str(response)
    ...
    def test_database_connectivity(self) -> bool:
        ...
        try:
            ...
            success = postgres_ok or mongo_ok or redis_ok  # At least one database should work
            details = f"PostgreSQL: {postgres_ok}, MongoDB: {mongo_ok}, Redis: {redis_ok}"
            
            db_info = {
                "postgresql": {"connected": postgres_ok, "error": postgres_error if not postgres_ok else None},
                "mongodb": {"connected": mongo_ok, "error": mongo_error if not mongo_ok else None},
                "redis": {"connected": redis_ok, "error": redis_error if not redis_ok else None}
            }
    ...
    def test_file_system(self) -> bool:
        ...
        try:
            ...
            success = all(check["exists"] for check in dir_checks.values()) and write_ok and read_ok
    ...
    def test_network_connectivity(self) -> bool:
        ...
        try:
            ...
            details = f"Internet: {internet_ok}, Localhost: {localhost_ok}, DNS: {dns_ok}"
