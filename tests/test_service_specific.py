...
class ServiceTestSuite:
    ...
    def test_llm_service(self) -> bool:
        ...
        try:
            ...
            self.log_test_result("Hardware Detection", True, f"Detected: {hardware_info}")
            
            # Test LLM service initialization
            llm_service = LocalLLMService()
            self.log_test_result("LLM Service Init", True, "Service initialized successfully")
            
            # Test model loading
            model_loaded = llm_service.load_model()
            self.log_test_result("Model Loading", model_loaded, "Model loaded successfully" if model_loaded else "Model loading failed")
            
            # Test basic inference
            if model_loaded:
                try:
                    response = llm_service.generate_response("Hello, how are you?", max_tokens=50)
                    self.log_test_result("Basic Inference", True, f"Response: {response[:100]}...")
            ...
            self.log_test_result("Company Context", context_loaded, "Context loaded successfully" if context_loaded else "Context loading failed")
    ...
    def test_document_service(self) -> bool:
        ...
        try:
            ...
            self.log_test_result("Document Processor Init", True, "Processor initialized")
            
            # Test enhanced document service
            doc_service = EnhancedDocumentService()
            self.log_test_result("Enhanced Document Service", True, "Service initialized")
            ...
            self.log_test_result("Document Validation", is_valid, "Document validation working")
    ...
    def test_auth_service(self) -> bool:
        ...
        try:
            ...
            self.log_test_result("Token Creation", bool(token), "Token created successfully")
            
            # Test token verification
            if token:
                verified = verify_token(token)
                self.log_test_result("Token Verification", bool(verified), "Token verified successfully")
            
            # Test password validation
            valid_password = validate_password("StrongPass123!")
            self.log_test_result("Password Validation", valid_password, "Password validation working")
    ...
    def test_file_service(self) -> bool:
        ...
        try:
            ...
            self.log_test_result("Document Utils", True, "Document utilities initialized")
            
            # Test file path operations
            test_path = "/tmp/test_file.txt"
            is_valid = file_utils.is_valid_file_path(test_path)
            self.log_test_result("File Path Validation", True, "Path validation working")
    ...
    def test_optimization_service(self) -> bool:
        ...
        try:
            ...
            self.log_test_result("Optimization Checks", True, f"Found {len(optimizations)} optimizations")
