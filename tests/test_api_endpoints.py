...
class APIEndpointTestSuite:
    ...
    def test_server_health(self) -> bool:
        ...
        try:
            ...
            self.log_test_result(
                "Server Health", success, details, response.json() if success else None
            )
            return success
    ...
    def test_authentication_endpoints(self) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/v2/auth/register", json=register_data
            )
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/v2/auth/login", json=login_data
            )
            success = response.status_code == 200
            ...
            self.log_test_result(
                "User Login", success, details, response.json() if success else None
            )
        ...
        if self.auth_token:
            try:
                ...
                response = self.session.post(
                    f"{self.base_url}/api/v2/auth/refresh", headers=headers
                )
                success = response.status_code == 200
    ...
    def test_document_endpoints(self) -> bool:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/v2/documents/list", headers=headers
            )
            success = response.status_code == 200
            ...
            self.log_test_result(
                "Document List", success, details, response.json() if success else None
            )
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/api/v2/documents/process",
    ...
    def test_llm_endpoints(self) -> bool:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/v2/llm/health", headers=headers
            )
            success = response.status_code == 200
            ...
            self.log_test_result(
                "LLM Health", success, details, response.json() if success else None
            )
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/v2/llm/query", json=query_data, headers=headers
            )
            success = response.status_code in [200, 202]  # 202 for async processing
            ...
            self.log_test_result(
                "LLM Query", success, details, response.json() if success else None
            )
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/v2/llm/analyze-document",
                json=analysis_data,
                headers=headers,
            )
    ...
    def test_financial_endpoints(self) -> bool:
        ...
        if not self.auth_token:
            self.log_test_result(
                "Financial Endpoints", False, "No auth token available"
            )
            return False
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/v2/financial/records", headers=headers
            )
            success = response.status_code == 200
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/v2/financial/summary", headers=headers
            )
            success = response.status_code == 200
    ...
    def test_analytics_endpoints(self) -> bool:
        ...
        if not self.auth_token:
            self.log_test_result(
                "Analytics Endpoints", False, "No auth token available"
            )
            return False
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/v2/analytics/dashboard", headers=headers
            )
            success = response.status_code == 200
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/v2/analytics/reports", headers=headers
            )
            success = response.status_code == 200
    ...
    def test_error_handling(self) -> bool:
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/api/v2/auth/login", json={"invalid": "data"}
            )
    ...
    def test_performance_endpoints(self) -> bool:
        ...
        if not self.auth_token:
            self.log_test_result(
                "Performance Endpoints", False, "No auth token available"
            )
            return False
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/v2/system/status", headers=headers
            )
            success = response.status_code == 200
            details = f"Status: {response.status_code}"
            ...
            self.log_test_result(
                "System Status", success, details, response.json() if success else None
            )
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/api/v2/system/metrics", headers=headers
            )
            success = response.status_code == 200
